...
public class DeletePauseItemsAction extends BaseAction
{
	private String str;
	...
	public String execute() throws Exception
	{
		...
		{
			List<Disable> dl = bookbns.getPbByIDs(bookIDs);
			...
			else
			{
				...
				for(Disable d:dl)
				{
					sltdate = d.getDisableDate();

					if(sltdate.compareTo(today) < 0)
					{
						setStr("s5 "+sltdate);	//该暂停日期已过，不能再删除
						return SUCCESS;
					}
					else
					{
						if((sltdate.compareTo(today) == 0) && (Integer.parseInt(d.getDisableTime().substring(0, 2)) < nowhour))
						{
							setStr("s6 "+d.getDisableTime());	//该暂停时间段已过，不能再删除
							return SUCCESS;
						}
					}
				}

				bookbns.deleteAllPb(dl);				
			}
		}
